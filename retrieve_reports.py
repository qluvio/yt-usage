...
if __name__ == '__main__':
  ...
  parser.add_argument('--report_url', default=None,
      help='URL of the report to retrieve. If not specified, the script ' +
           ...
           'selected job.')
  parser.add_argument('--local_file', default='reports.csv',
  ...
  try:
    ...
    for report in reports:
        i += 1
        if i%100 == 0: # 100 free requests per 100 seconds for report API
            time.sleep(100)
        # report['startTime'], report['endTime'], report['downloadUrl']
        args.report_url = report['downloadUrl']
        if args.report_url:
